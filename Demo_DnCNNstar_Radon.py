from DataFidelities.RadonEffClass import RadonEffClass
...
for i in range(startIndex,endIndex):
    ...
    mask = sio.loadmat('maskes/mask{}_img{}.mat'.format(number_projections, i)).get('img')

    #-- Reconstruction --#
    dObj = RadonEffClass(y, sigSize, mask, theta=theta, num_blocks=num_blocks, block_size=block_size)
    rObj = DnCNNClass(sigSize, tau, model_path, img_channels=input_channels, truth_channels=truth_channels, mode='SIM')

    # you can check the LC of the neural net by using the spectral-normalization
    # in Sedghi et al 'Singular Values of Convolutional Layers'
    # by commenting out the following line of code.
    # Note that the code is not compatible with CPU programming.

    # rObj.lipschitz_upper()
    ...
    # We set the step-size to be 1/(Lmax+2*tau), where Lmax <= L
    bcred_recon, bcred_out = bcredEst(dObj, rObj,
                            num_patch=num_blocks, patch_size=block_size, pad=10, numIter=iters, step=1/(70+2*tau),
    ...
    red_recon, red_out = redEst(dObj, rObj,
                        numIter=iters, step=1/(250+2*tau), accelerate=False, mode='RED', useNoise=True,
